...
def generate_article(deal, press_text, financials, analysis, text_blocks):
    ...
    prompt = f"""M&A専門メディア「J-MAT」のシニアアナリストとして、以下の案件の分析記事を書いてください。

【案件名】{deal['title']}
{info_section}
{f'【財務データ】{chr(10)}{fin_summary}' if fin_summary else ''}
{nums_info}

以下の3セクションで出力。各セクション100〜150字。合計500字以内。Markdown ##見出しで区切る。

## 案件概要
買い手・売り手の名称、証券コード、スキーム、取得価額、取得日、売り手業績（売上高・営業利益・純資産）を箇条書きで整理。

## 戦略的背景とシナジー
買収理由、対象企業の強み、期待されるシナジーを具体的に。テンプレ表現（「シナジー効果が期待される」等）は禁止。具体的な事業領域名や数値で論じること。

## J-MAT総合評価
戦略的意義の総括、PMI課題、ウォッチポイントを1段落で。必ず文を完結させること。"""
...
def generate_analysis_comment(deal, financials, text_blocks, companies, press_text=""):
    ...
    # 業種別マルチプル参考値
    industry_ref = get_industry_multiples(deal.get('_industry', ''))

    # 案件タイプ別の追加分析指示
    deal_type_instruction = get_deal_type_instruction(deal['title'])

    prompt = f"""M&A専門アナリストとして、以下のデータに基づき財務分析コメントを300字以内で書いてください。

【案件】{deal['title']}
【対象企業】{company_names if company_names else '不明'}
{f'【EDINET財務データ】{chr(10)}{fin_text}' if fin_text else ''}
{press_nums}
{val_text}
{industry_ref}

以下の3点を各2〜3文で簡潔に述べること。見出し番号は不要。段落で区切ること。

第1段落：バリュエーション評価。算出された指標を業種参考値と比較し、割高・割安・妥当のいずれかを判定する。参考値がない場合は一般的な水準（EV/EBITDA 8-12xが中央値帯）と比較する。
第2段落：売り手の収益性と買い手の買収余力。営業利益率の水準感、取得価額が買い手の財務に与える影響を述べる。
第3段落：PMIと今後の注目点。{deal_type_instruction}

【絶対ルール】
- 「業界水準との比較は確認できない」というフレーズは使用禁止。必ず参考値または一般水準と比較すること。
- 300字以内に収めること。全文を完結させること。途中で切らないこと。"""
...
# ======================
INDUSTRY_MULTIPLES = {
    "情報サービス業":           {"ev_ebitda": "15-25x", "ev_rev": "2-5x", "note": "SaaS/IT企業は高倍率が一般的"},
    "インターネット附随サービス業": {"ev_ebitda": "15-30x", "ev_rev": "3-8x", "note": "EC/プラットフォーム企業は成長性で評価"},
    "医薬品製造業":             {"ev_ebitda": "12-20x", "ev_rev": "3-6x", "note": "パイプライン価値を含む"},
    "化学工業":                 {"ev_ebitda": "8-12x", "ev_rev": "1-2x", "note": "化粧品・コスメは高めの傾向"},
    "機械器具製造業":           {"ev_ebitda": "8-12x", "ev_rev": "0.8-1.5x", "note": "半導体関連は高めの傾向"},
    "電気機械器具製造業":       {"ev_ebitda": "10-15x", "ev_rev": "1-3x", "note": "半導体関連は高倍率"},
    "建設業":                   {"ev_ebitda": "5-8x", "ev_rev": "0.3-0.8x", "note": "受注残高と利益率が重要"},
    "飲食サービス業":           {"ev_ebitda": "8-15x", "ev_rev": "0.8-2x", "note": "店舗数と既存店成長率が評価軸"},
    "小売業":                   {"ev_ebitda": "7-12x", "ev_rev": "0.5-1.5x", "note": "業態により大きく異なる"},
    "不動産業":                 {"ev_ebitda": "10-15x", "ev_rev": "2-5x", "note": "NAV倍率も重要指標"},
    "広告・マーケティング業":   {"ev_ebitda": "8-15x", "ev_rev": "1-3x", "note": "デジタル比率で評価が変動"},
    "人材サービス業":           {"ev_ebitda": "8-12x", "ev_rev": "0.8-2x", "note": "リカーリング収益比率が重要"},
    "介護・社会福祉業":         {"ev_ebitda": "8-12x", "ev_rev": "0.5-1.5x", "note": "許認可と施設数が評価軸"},
    "医療業":                   {"ev_ebitda": "8-12x", "ev_rev": "1-2x", "note": "患者数と診療報酬改定の影響"},
    "食料品製造業":             {"ev_ebitda": "8-12x", "ev_rev": "0.8-1.5x", "note": "ブランド力と販路が重要"},
    "運輸業":                   {"ev_ebitda": "6-10x", "ev_rev": "0.5-1x", "note": "車両等の有形資産が多い"},
    "廃棄物処理業":             {"ev_ebitda": "8-12x", "ev_rev": "1-2x", "note": "許認可ビジネスで参入障壁高い"},
    "その他金融業":             {"ev_ebitda": "8-15x", "ev_rev": "2-5x", "note": "AUM・運用残高が評価軸"},
    "銀行業":                   {"ev_ebitda": "N/A", "ev_rev": "N/A", "note": "PBR 0.3-0.8xが国内平均"},
    "木材・木製品製造業":       {"ev_ebitda": "5-8x", "ev_rev": "0.5-1x", "note": "住宅着工件数との連動性高い"},
}

def get_industry_multiples(industry):
    """業種別の参考マルチプルをプロンプト用テキストで返す"""
    m = INDUSTRY_MULTIPLES.get(industry)
    if m:
        lines = [f"【業種参考マルチプル：{industry}】"]
        if m["ev_ebitda"] != "N/A":
            lines.append(f"EV/EBITDA中央値帯: {m['ev_ebitda']}")
        if m["ev_rev"] != "N/A":
            lines.append(f"EV/売上高中央値帯: {m['ev_rev']}")
        lines.append(f"備考: {m['note']}")
        return "\n".join(lines)
    return "【業種参考マルチプル】一般的なM&A市場のEV/EBITDA中央値帯は8-12x程度"

def get_deal_type_instruction(title):
    """案件タイプに応じた分析の切り口を返す"""
    if "TOB" in title or "公開買付" in title:
        return "TOBのため少数株主保護・買付価格の公正性・応募見通しに言及すること。"
    elif "MBO" in title:
        return "MBOのため経営陣の利益相反・第三者委員会の設置・公正性担保措置に言及すること。"
    elif "経営統合" in title or "合併" in title:
        return "経営統合のため統合比率の妥当性・重複事業の整理・独禁法審査の見通しに言及すること。"
    elif any(k in title for k in ["米国", "欧州", "中国", "アジア", "海外", "フィリピン", "ハンガリー"]):
        return "クロスボーダー案件のため為替リスク・現地規制・PMIの難易度に言及すること。"
    else:
        return "のれん計上額の見通し・買収後の業績寄与時期・統合シナジーの実現可能性に言及すること。"

def calc_jmat_rating(art):
    """J-MATレーティング（A/B/C）を算出"""
    score = 0
    val = art.get("valuation", {})
    raw = val.get("raw", {})

    # 情報開示の充実度
    if raw.get("acquisition_price"): score += 2
    if raw.get("revenue"): score += 1
    if raw.get("operating_income"): score += 1
    if art.get("article_body") and len(art["article_body"]) > 200: score += 1

    # バリュエーションの妥当性
    ev_ebitda = val.get("ev_ebitda_approx", 0)
    if ev_ebitda and 5 <= ev_ebitda <= 20: score += 2  # 妥当レンジ
    elif ev_ebitda and ev_ebitda > 30: score -= 1  # 割高懸念

    op_margin = val.get("op_margin", 0)
    if op_margin and op_margin >= 10: score += 1  # 高収益
    if op_margin and op_margin >= 20: score += 1  # 非常に高収益

    # 戦略性
    title = art.get("title", "")
    if any(k in title for k in ["TOB", "MBO", "経営統合"]): score += 1
...
for art in featured_data:
    ...
    # J-MATレーティング算出
    art["rating"] = calc_jmat_rating(art)
    time.sleep(5)
...
for f in featured_data:
    ...
    safe_analysis = safe_analysis[:200]
...
for art in featured_data:
    ...
    rating_emoji = {"A": "🟢A", "B": "🟡B", "C": "⚪C"}.get(rating, "⚪C")
